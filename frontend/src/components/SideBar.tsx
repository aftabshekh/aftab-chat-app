...
function SideBar() {
    ...
    useEffect(() => {
        const handleClick = (e: Event) => {
            if (div.current?.contains(e.target as Node)) return;
            setDisplay(false);
        };
        ...
        <div className='flex items-start justify-center'>
            <div ref={side} className='bg-[#3e3c61] p-5 w-fit h-[500px] rounded-bl-lg rounded-tl-lg mobile:invisible mobile:absolute z-40 mobile:right-[50%]'>
                ...
                <div className='flex items-center gap-1'>
                    <img className='w-10 rounded-full object-cover' src={isEmpty ? avatar : currentUser.displayPicture} alt="user" />
                    <p>{currentUser.userName}</p>
                </div>

                {display && (
                    <div className='fixed left-1/2 mobile:w-80 -translate-x-1/2 z-10 bg-primary px-6 py-4 rounded-md'>
